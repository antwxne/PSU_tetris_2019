...
void init_window(game_t *game)
{
    ...
    game->windows[INFO] = subwin(stdscr, 20, 50, LINES / 2 - 10 , 0);
    game->windows[NEXT] = subwin(stdscr, game->size_b.y+2, game->size_b.x+2,
    LINES / 2 - game->size_b.y / 2 - 10, COLS / 2 + (game->size_b.x / 2) + 10);
    keypad(stdscr, TRUE);
